...
收藏夹id、用户id、收藏夹名字、收藏数量、创建时间、修改时间、是否删除

id、user_id、favorite_name、collect_number、create_time、update_time、is_deleted

```sql
CREATE TABLE `favorite` (
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '收藏夹id',
`user_id` VARCHAR(20) COMMENT '用户id',
`favorite_name` VARCHAR(20) COMMENT '收藏夹名字',
`collect_number` VARCHAR(20) COMMENT '收藏数量',    
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
收藏id、用户id、收藏夹id、内容id、创建时间、修改时间、是否删除

id、user_id、favorite_id、content_id、create_time、update_time、is_deleted

```sql
CREATE TABLE `collect` (
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '收藏id',
`user_id` VARCHAR(20) COMMENT '用户id',    
`favorite_id` VARCHAR(20) COMMENT '收藏夹id',
`content_id` VARCHAR(20) COMMENT '内容id',
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
文章id、用户id、内容、赞同数、反对数、创建时间、修改时间、是否删除

id、user_id、content、approval_number、oppose_number、create_time、update_time、is_deleted

```sql
CREATE TABLE `article` (
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '文章id',
`user_id` VARCHAR(20) COMMENT '用户id',   
`content` TEXT COMMENT '内容',
`approval_number` VARCHAR(10) COMMENT '赞同数',
`oppose_number` VARCHAR(10) COMMENT '反对数',
...
举报id、举报者id、文章id、举报理由、举报详情、是否处理、处理者id、处理结果、创建时间、修改时间、是否删除
