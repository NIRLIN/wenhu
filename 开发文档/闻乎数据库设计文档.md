...
记录id、文章或回答id、文章或回答、用户id、创建时间、修改时间、是否删除

id、user_id、answer_article_id、answer_article、create_time、update_time、is_deleted

```sql
CREATE TABLE `browsing` (
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '记录id',
`user_id` VARCHAR(20) COMMENT '用户id',    
`answer_article_id` VARCHAR(20) COMMENT '文章或回答id',
`answer_article` tinyint(1) DEFAULT 1 COMMENT '文章或回答，1为回答，0为文章',      
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
用户id、回答、问题、文章、收藏、关注、创建时间、修改时间、是否删除
...
问题id、用户id、问题标题、描述、关注数、浏览数、创建时间、修改时间、是否删除

id、user_id、title、description、follow_number、browse_number、create_time、update_time、is_deleted
...
`browse_number` INT(10) COMMENT '浏览数',    
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
id、mender_id、question_id、is_title、update_before_content、update_after_content、create_time、update_time、is_deleted
...
回答id、问题id、回答或文章、用户id、内容、赞同数、反对数、创建时间、修改时间、是否删除

id、question_id、answer_article、user_id、content、approval_number、oppose_number、create_time、update_time、is_deleted
...
赞同或反对id、回答或文章id、赞同或反对、用户id、创建时间、修改时间、是否删除

id、answer_article_id、agree_oppose、user_id、create_time、update_time、is_deleted

```sql
CREATE TABLE `agree_oppose` (
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '赞同或反对id',
`answer_article_id` VARCHAR(20) COMMENT '回答或文章id',
...
评论id、回答或文章id、回答或文章、评论者id、评论内容、创建时间、修改时间、是否删除

id、answer_article_id、reviewer_id、review_content、create_time、update_time、is_deleted
...
举报id、举报者id、问题id、举报理由、举报详情、是否处理、处理者id、处理结果、创建时间、修改时间、是否删除
...
`question_id` VARCHAR(20) COMMENT '问题id',
`report_reason` VARCHAR(200) COMMENT '举报理由',
`report_details` VARCHAR(200) COMMENT '举报详情', 
`is_dispose` tinyint(1) DEFAULT 0 COMMENT '是否处理',
`disposer_id` VARCHAR(20) COMMENT '处理者id',
`dispose_result` VARCHAR(200) COMMENT '处理结果',       
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
举报id、举报者id、回答或文章id、举报理由、举报详情、是否处理、处理者id、处理结果、创建时间、修改时间、是否删除
...
```sql
CREATE TABLE `report_answer_article` (
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '举报id',
`reporter_id` VARCHAR(20) COMMENT '举报者id',
`answer_article_id` VARCHAR(200) COMMENT '回答或文章id',
`report_reason` VARCHAR(200) COMMENT '举报理由',
`report_details` VARCHAR(200) COMMENT '举报详情',
`is_dispose` tinyint(1) DEFAULT 0 COMMENT '是否处理',
`disposer_id` VARCHAR(20) COMMENT '处理者id',
...
收藏id、用户id、回答或文章id、文章或回答、创建时间、修改时间、是否删除
