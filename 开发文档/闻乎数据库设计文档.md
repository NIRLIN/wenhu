...
`answer_article_id` VARCHAR(20) COMMENT '浏览id',
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
`question_heat` INT  COMMENT '问题热度',
...
`approval_number` INT COMMENT '赞同数',
`oppose_number` INT COMMENT '反对数',
`create_time` datetime COMMENT '创建时间',
`update_time` datetime COMMENT '修改时间',
`is_deleted` tinyint(1) DEFAULT 0 COMMENT '是否删除'
)ENGINE = INNODB DEFAULT CHARSET = utf8mb4;
...
赞同或反对id、回答OR文章id、赞同或反对、用户id、创建时间、修改时间、是否删除

id、parent_id、agree_oppose、user_id、create_time、update_time、is_deleted
...
评论id、评论者id、评论内容、文章或回答、父回答OR文章id、父评论id、创建时间、修改时间、是否删除

id、reviewer_id、review_content、answer_article、parent_id、parent_review_id、create_time、update_time、is_deleted
...
`id` VARCHAR(20) NOT NULL PRIMARY KEY COMMENT '举报id',
`reporter_id` VARCHAR(20) COMMENT '举报者id',
`answer_article_id` VARCHAR(20) COMMENT '回答id',
`report_reason` VARCHAR(20) COMMENT '举报理由',
`report_details` VARCHAR(20) COMMENT '举报详情',
`is_dispose` tinyint(1) DEFAULT 0 COMMENT '是否处理',
`disposer_id` VARCHAR(20) COMMENT '处理者id',
